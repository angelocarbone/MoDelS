...
Key shortcuts
    ...
    TAB:         Move camera to next cf_vehicle
    ...
    Arrow keys is used to drive externally controlled Ego cf_vehicle:
        Up:    Accelerate
        Down : Brake
        Left : Steer left
        Right : Steer right
    ...
    When custom camera model(1) is activated
        k: Switch between the following sub models:
           - Orbit        (camera facing cf_vehicle, rotating around it)
           - Fixed        (fix rotation, always straight behind cf_vehicle)
           - Flex         (imagine the camera attached to cf_vehicle via an elastic string)
           - Flex - orbit (Like flex but allows for roatation around cf_vehicle)
           - Top          (top view, fixed rotation, always straight above cf_vehicle)
           - Driver       ("cf_driver" view, fixed at center of cf_vehicle)
