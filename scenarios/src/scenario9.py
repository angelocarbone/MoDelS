...
class Scenario(ScenarioGenerator):
    ...
    def base_scenario_factory(self, save_scenario_file=False, print_scenario=False, injected_init_function=None,
    ...
    # + @add_init_action
    def create_init_jam_vehicle(self, lane_position_s=120, lane_position_offset=0, lane_position_lane_id=-1,
                                lane_position_road_id=2, orientation: xosc.Orientation = None, absolute_speed_action=10):
        step_time = xosc.TransitionDynamics(xosc.DynamicsShapes.step, xosc.DynamicsDimension.time, 1)
        lane_position = xosc.LanePosition(lane_position_s, lane_position_offset, lane_position_lane_id,
                                          lane_position_road_id)
    ...
    def _create_event_pedestrian_walk_event(self):
        ...
        a_time = [0, 0, 0, 0, 0, 0]
        positions = [
            xosc.LanePosition(250, 0.5, 3, 2, xosc.Orientation(0, 0, 0, xosc.ReferenceContext.relative)),
            xosc.LanePosition(245.5, 0.5, 3, 2, xosc.Orientation(0, 0, 0, xosc.ReferenceContext.relative)),
            xosc.LanePosition(245, 0.0, 3, 2, xosc.Orientation(1.57, 0, 0, xosc.ReferenceContext.relative)),
            xosc.LanePosition(245, 0.0, -3, 2, xosc.Orientation(4.71, 0, 0, xosc.ReferenceContext.relative)),
            xosc.LanePosition(244.5, -0.5, -3, 2, xosc.Orientation(3.14, 0, 0, xosc.ReferenceContext.relative)),
            xosc.LanePosition(235, -0.5, -3, 2, xosc.Orientation(3.14, 0, 0, xosc.ReferenceContext.relative)),
        ]
    ...
    def create_event_jam_vehicle(self):
        ...
        #                                    xosc.TriggeringEntitiesRule.any
        #                                    )
        jam_condition = xosc.ValueTrigger('{0}_Condition'.format(prefix), 0.0, xosc.ConditionEdge.rising,
                                          value_condition)
        ...
        jam_transition_dynamic = xosc.TransitionDynamics(xosc.DynamicsShapes.sinusoidal,
