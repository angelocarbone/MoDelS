...
ego_start = xosc.TeleportAction(xosc.LanePosition(25, 0, -3, 0))
...
vehicle_start = xosc.TeleportAction(xosc.LanePosition(30, 0, -3, 0))
...
trigger_condition = xosc.RelativeDistanceCondition(2, xosc.Rule.lessOrEqual, xosc.RelativeDistanceType.longitudinal, vehicle_name)
...
transition_dynamic = xosc.TransitionDynamics(xosc.DynamicsShapes.cubic, xosc.DynamicsDimension.time, 5)
action = xosc.AbsoluteLaneChangeAction(-2, transition_dynamic)
...
story_param.add_parameter(xosc.Parameter('$owner', xosc.ParameterType.string, ego_name))
...
# create the scenario
sce = xosc.Scenario('AdaptSpeedExample', 'Angelo Carboπe', param_declaration, entities=entities, storyboard=sb, roadnetwork=road, catalog=catalog)
